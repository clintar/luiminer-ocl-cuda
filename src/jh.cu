...
__constant__ unsigned char E8_bitslice_roundconstant[42][32]={
...
__device__ void Sbox_and_MDS_layer(uint32 state[8][4], uint32 roundnumber)
{
	uint32 temp0;
	uint32 cc0, cc1;
	//Sbox and MDS layer
#pragma unroll 4
	for (int i = 0; i < 4; i++) {
		cc0 = ((uint32*)E8_bitslice_roundconstant[roundnumber])[i];
		cc1 = ((uint32*)E8_bitslice_roundconstant[roundnumber])[i+4];
		Sbox(state[0][i],state[2][i], state[4][i], state[6][i], cc0);
		Sbox(state[1][i],state[3][i], state[5][i], state[7][i], cc1);
		L(state[0][i],state[2][i],state[4][i],state[6][i],state[1][i],state[3][i],state[5][i],state[7][i]);
	}
}

__device__ void RoundFunction0(uint32 state[8][4], uint32 roundnumber)
{
...
__device__ void RoundFunction1(uint32 state[8][4], uint32 roundnumber)
{
	Sbox_and_MDS_layer(state, roundnumber);
...
__device__ void RoundFunction2(uint32 state[8][4], uint32 roundnumber)
{
	Sbox_and_MDS_layer(state, roundnumber);
...
__device__ void RoundFunction3(uint32 state[8][4], uint32 roundnumber)
{
	Sbox_and_MDS_layer(state, roundnumber);
...
__device__ void RoundFunction4(uint32 state[8][4], uint32 roundnumber)
{
	Sbox_and_MDS_layer(state, roundnumber);
...
__device__ void RoundFunction5(uint32 state[8][4], uint32 roundnumber)
{
	uint32 temp0;
...
__device__ void RoundFunction6(uint32 state[8][4], uint32 roundnumber)
{
	uint32 temp0;
...
__global__ void kernelJh512(unsigned char *hashes) {

	uint32 x[8][4];     /*the 1024-bit state, ( x[i][0] || x[i][1] || x[i][2] || x[i][3] ) is the ith row of the state in the pseudocode*/
	uint32 buffer[16];  /*the 512-bit message block to be hashed;*/
...
#define INIT(a,b,c,d) ((a) | ((b)<<8) | ((c)<<16) | ((d)<<24))
	...
	for (i = 0; i < 16; i++) 
		buffer[i] = dec32le2(hashes + (tid*64) + (i << 2));
	...
	for (i = 0; i < 16; i++)
		enc32le2(hashes + (tid*64) + (i << 2), x[4][i]);

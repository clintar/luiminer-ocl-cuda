...
static size_t resp_hdr_cb(void *ptr, size_t size, size_t nmemb, void *user_data)
{
    ...
    ptrlen = size * nmemb;
    val = (char *)xcalloc(1, ptrlen);
    key = (char *)xcalloc(1, ptrlen);
    if (!key || !val)
        goto out;
...
static void stratum_buffer_append(struct stratum_ctx *sctx, const char *s)
{
    ...
    nnew = old + strlen(s) + 1;
    if (nnew >= sctx->sockbuf_size) {
        sctx->sockbuf_size = nnew + (RBUFSIZE - (nnew % RBUFSIZE));
...
bool stratum_connect(struct stratum_ctx *sctx, const char *url)
{
    ...
    curl = sctx->curl;
    if (!sctx->sockbuf) {
        sctx->sockbuf = (char *)xcalloc(RBUFSIZE, 1);
...
start:
    ...
    sctx->xnonce1 = (unsigned char *)xmalloc(sctx->xnonce1_size);
...
static bool stratum_notify(struct stratum_ctx *sctx, json_t *params)
{
    ...
    merkle = (unsigned char **)xmalloc(merkle_count * sizeof(char *));
    for (i = 0; i < merkle_count; i++) {
        ...
        merkle[i] = (unsigned char *)xmalloc(32);
...
struct thread_q *tq_new(void)
{
    ...
    tq = (thread_q *)xcalloc(1, sizeof(*tq));
    INIT_LIST_HEAD(&tq->q);
...
bool tq_push(struct thread_q *tq, void *data)
{
    ...
    ent = (struct tq_ent *)xcalloc(1, sizeof(*ent));
    ent->data = data;

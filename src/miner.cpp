...
#define ALGO_COUNT ARRAY_SIZE(algo_names)
...
static char const usage [] =
    ...
    -a, --algo=ALGO      specify the algorithm to use\n\
		...
		x11_ocl   X11 OpenCL\n\
		x11_cuda  X11 CUDA\n\
    -d, --device=N       start GPU device to use (default: 0) \n\
    -i, --intensity=N    GPU work intensity (default: 18) \n\
	...
	-t, --threads=N      number of miner threads\n\
			 default for cpu: number of processors\n\
			 default for gpu: number of active GPUs\n\
	...
	int recur)
{
	...
	json_t *res = json_rpc_call(curl,
		url,
		userpass,
		json_dumps(rpc_req, 0),
		curl_err,
	...
	if (!strcmp(mes, "Unauthenticated")) {
		...
		return json_rpc2_call_recur(curl,
			url,
			userpass,
			rpc_req,
			curl_err,
...
static inline void work_copy(struct work *dest, const struct work *src) {
	...
	if (src->xnonce2) {
		dest->xnonce2 = (unsigned char *)xmalloc(src->xnonce2_len);
		memcpy(dest->xnonce2, src->xnonce2, src->xnonce2_len);
	}
}

static bool jobj_binary(const json_t *obj,
	const char *key,
	void *buf,
	size_t buflen)
{
...
bool rpc2_job_decode(const json_t *job, struct work *work)
{
	...
	if (blobLen != 0)
	{
		...
		char *blob = (char *)xmalloc(blobLen / 2);
		...
		rpc2_blob = (char *)xmalloc(rpc2_bloblen);
...
static void share_result(int result, struct work *work, const char *reason) {
	...
	applog(LOG_INFO,
		"eff: %.0f%% @ %.0f kh/s, accepted: %lu/%lu (%.2f%%), %.0f kh/s at diff %.0f %s",
...
static bool submit_upstream_work(CURL *curl, struct work *work) {
	...
	if (have_stratum) {
		...
		for (i = 0; i < work->noncecnt; ++i)
		{
			if (jsonrpc_2) {
				uint8_t hash[32];
				char *hashhex;
				
				noncestr = bin2hex((uint8_t *)(work->nonces + i), 8);
				strcpy(last_found_nonce, noncestr);
				memcpy(((uint8_t *)work->data) + 1, work->nonces + i, 8);
				x11_hash(hash, work->job_len, (uint8_t*)work->data);
				hashhex = bin2hex((const unsigned char *)hash, 32);
				snprintf(s,
					JSON_BUF_LEN,
...
static bool workio_get_work(struct workio_cmd *wc, CURL *curl) {
	...
	ret_work = (work*)xcalloc(1, sizeof(*ret_work));
	if (!ret_work)
		return false;
...
static bool get_work(struct thr_info *thr, struct work *work) {
	...
	wc = (workio_cmd*)xcalloc(1, sizeof(*wc));
	if (!wc)
		return false;
	wc->cmd = WC_GET_WORK;
	wc->thr = thr;
...
static bool submit_work(struct thr_info *thr, const struct work *work_in) {
	...
	wc->u.work = (work*)xmalloc(sizeof(*work_in));
	if (!wc->u.work)
		goto err_out;
...
static void *miner_thread(void *userdata) {
	...
	struct work work = {{0}};
	uint64_t max_nonce;
	uint64_t end_nonce = 0xFFFFFFFFFFFFFFFFUL / opt_n_threads * (thr_id + 1) - 0x20;
	char s[16];
	int i;
	...
	if (opt_algo == ALGO_X11_OCL)
		mythr->gpu_ocl->curnonce = *nonceptr;		// new work, reset it
	if (opt_algo == ALGO_X11_CUDA)
		mythr->gpu_cuda->curnonce = *nonceptr;
	...
	while (1) {
		...
		if (memcmp(((uint8_t*) work.data) + 1 + 8, ((uint8_t*) g_work.data) + 1 + 8, 80 - 9)) {
			...
			*nonceptr = 0xffffffffU / opt_n_threads * thr_id;
			if (opt_algo == ALGO_X11_OCL)
				mythr->gpu_ocl->curnonce = *nonceptr;
			if (opt_algo == ALGO_X11_CUDA)
				mythr->gpu_cuda->curnonce = *nonceptr;
		}
		else {
			if (opt_algo == ALGO_X11_OCL)
				*nonceptr = mythr->gpu_ocl->curnonce;
			if (opt_algo == ALGO_X11_CUDA)
				*nonceptr = mythr->gpu_cuda->curnonce;
		...
		if (opt_debug) {
			applog(LOG_DEBUG,
				"Thread %d is going to scan with start nonce=0x%08x, end_nonce=0x%08x",
				thr_id,
				*nonceptr,
				max_nonce);
		}
		...
		if (opt_algo == ALGO_X11)
			rc = scanhash_x11_jsonrpc_2(thr_id, work.data, work.target, max_nonce, (uint64_t *)&hashes_done);
		else if (opt_algo == ALGO_X11_OCL)
			rc = scanhash_x11_jsonrpc_2_ocl(thr_id, mythr->gpu_ocl, &work, max_nonce, &hashes_done);
		else if (opt_algo == ALGO_X11_CUDA)
			rc = scanhash_x11_jsonrpc_2_cuda(thr_id, mythr->gpu_cuda, &work, max_nonce, &hashes_done);
...
out:
	...
	static void parse_arg(int key, char *arg) {
			...
			if (p) {
				...
				*p = '\0';
				...
				if (sp) {
					...
					free(rpc_user);
					rpc_user = (char *)xcalloc(sp - ap + 1, 1);
		...
		case 'O': /* --userpass */
			...
			free(rpc_user);
			rpc_user = (char *)xcalloc(p - arg + 1, 1);
...
/*#ifndef WIN32
	...
	work_restart = (struct work_restart *)xcalloc(opt_n_threads, sizeof(*work_restart));
	if (!work_restart)
		return 1;

	thr_info = (struct thr_info *)xcalloc(opt_n_threads + 3, sizeof(*thr));
	if (!thr_info)
		return 1;

	thr_hashrates = (double *)xcalloc(opt_n_threads, sizeof(double));
	if (!thr_hashrates)
		return 1;
	...
	if (opt_algo == ALGO_X11_OCL) {
		for (i = 0; i < opt_n_threads; i++) {
			thr = &thr_info[i];
																/* (O_o) */
			thr->gpu_ocl = clInitGpu(i + opt_device, opt_algo == ALGO_X11_OCL ? 0 : 1);
			if (thr->gpu_ocl == NULL)
				break;
		}
	...
	if (opt_algo == ALGO_X11_CUDA) {
		...
		for (i = 0; i < opt_n_threads; i++) {
			thr = &thr_info[k];
			thr->gpu_cuda = cuInitGpu(i + opt_device, ARCH_20);
			if (thr->gpu_cuda == NULL)
				continue;
			k++;
		}
		opt_n_threads = k;

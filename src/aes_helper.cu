	...
	uint32 &y0, uint32 &y1, uint32 &y2, uint32 &y3)
{
	...
	idx0 = bytePerm2(x0, 0, 0x4440);
	idx1 = bytePerm2(x1, 0, 0x4441);
	idx2 = bytePerm2(x2, 0, 0x4442);
	idx3 = bytePerm2(x3, 0, 0x4443);
	y0 =AES0[idx0] ^ 
		AES1[idx1] ^ 
		AES2[idx2] ^ 
		AES3[idx3]; // ^k0;

	idx0 = bytePerm2(x1, 0, 0x4440);
	idx1 = bytePerm2(x2, 0, 0x4441);
	idx2 = bytePerm2(x3, 0, 0x4442);
	idx3 = bytePerm2(x0, 0, 0x4443);
	...
	idx0 = bytePerm2(x2, 0, 0x4440);
	idx1 = bytePerm2(x3, 0, 0x4441);
	idx2 = bytePerm2(x0, 0, 0x4442);
	idx3 = bytePerm2(x1, 0, 0x4443);
	y2 =AES0[idx0] ^ 
		AES1[idx1] ^ 
		AES2[idx2] ^ 
		AES3[idx3]; // ^k2

	idx0 = bytePerm2(x3, 0, 0x4440);
	idx1 = bytePerm2(x0, 0, 0x4441);
	idx2 = bytePerm2(x1, 0, 0x4442);
	idx3 = bytePerm2(x2, 0, 0x4443);
	y3 =AES0[idx0] ^ 
		AES1[idx1] ^ 
		AES2[idx2] ^ 
		AES3[idx3]; // ^k3
...
static __device__ __forceinline__ void aes_roundK(
	uint32 x0, uint32 x1, uint32 x2, uint32 x3, 
	uint32 k0,
	uint32 &y0, uint32 &y1, uint32 &y2, uint32 &y3)
{
